...
class bnEditStudentActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        studentImageView.setImageResource(R.drawable.student_default) // ✅ תמונה קבועה לכל הסטודנטים

        // ✅ כפתור שמירה - מעדכן את הנתונים ומחזיר אותם ל-`MainActivity`
        ...
        // ✅ כפתור ביטול - לא משנה כלום וחוזר לרשימה
        cancelButton.setOnClickListener {
            setResult(Activity.RESULT_CANCELED)
            finish()
        }

        // ✅ כפתור חזרה - פשוט סוגר את המסך ללא שינוי
        backButton.setOnClickListener {
            setResult(Activity.RESULT_CANCELED)
            finish()
        }

        // ✅ כפתור מחיקה - מוחק את הסטודנט מהרשימה ומחזיר את הנתונים
